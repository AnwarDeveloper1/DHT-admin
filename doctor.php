...
if (isset($_POST['addDoctor'])) {
  ...
  $stmt = $conn->prepare("INSERT INTO doctor (d_name, d_contact, specialization, deptt_id) VALUES (?, ?, ?, ?)");
  $stmt->bind_param("sssi", $name, $contact, $specialization, $department_id);
  $stmt->execute();
  $stmt->close();
...
if (isset($_POST['updateDoctor'])) {
  ...
  $stmt = $conn->prepare("UPDATE doctor SET d_name=?, d_contact=?, specialization=?, deptt_id=? WHERE doctor_id=?");
  $stmt->bind_param("sssii", $name, $contact, $specialization, $department_id, $id);
  $stmt->execute();
  $stmt->close();
  header("Location: doctor.php");
  exit();
